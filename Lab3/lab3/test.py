...
def test_complex_json():
    value=[None,{'key1':'value1',2:True,'???':None},set([None,'fldfkndsfdsfm',34.0023442,True,'wejwie',False]),[{'key':['value1','value2']},(None,'fldfkndsfdsfm',34.0023442,True,'wejwie',False)]]
    serializer=Json()
    packed=serializer.dumps(value)
    assert value == serializer.loads(packed)
...
def test_arg_n_kwarg_func_json():
    ...
    assert arg_n_kwarg_func('first','second',kwarg2=False,kwarg1=True)==serializer.loads(packed)('first','second',kwarg2=False,kwarg1=True)
